...
async def resolve_place_context(query: str) -> PlaceContext:
    ...
    if prefer_keyword and not juso_first:
        from poi_resolver import resolve_dynamic_poi

        poi = await resolve_dynamic_poi(stripped)
        if poi:
            ctx.apply_coords(poi.latitude, poi.longitude)
            ctx.apply_admin(sido=poi.sido, sigungu=poi.sigungu, dong=poi.dong)
            ctx.poi_name = poi.place_name
            ctx.source = poi.source
            ctx.expanded_query = poi.road_address or f"{poi.place_name} {poi.sigungu}".strip()
            _set_confidence(ctx, "high")
    ...
    if (not juso_first or not ctx.coords) and ctx.source != "kakao_poi":
